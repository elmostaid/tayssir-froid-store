...
export default async function AdminReportsPage({
  ...
  // دفعات من ثلاثة، لا سبعة دفعةً واحدة.
  //
  // كانت هذه الصفحة تُطلق خمسة استعلامات متوازية ثم اثنين متسلسلين، وفوقها
  // استعلام الـlayout الذي يُصيَّر بالتوازي — سبعة طلبات اتصال على مجمّع
  // سعته خمسة. الحدّ هنا ثلاثة، فيبقى للـlayout سلوت وللمجمّع احتياطي
  // (التفصيل في lib/admin/sectionData.ts). والترتيب مقصود: الدفعة الأولى
  // تحمل ما يُقرأ أولاً في الصفحة.
  //
  // ولا يُستبدَل فشلٌ برقم: كل استعلام يعود إما بقيمته وإما بتعذُّر مُعلَن
  // يعرضه قسمه وحده. «الربح الخام 0,00 درهم» ليست معرفة ناقصة بل معلومة
  // خاطئة يُقرَّر عليها.
  const [
    salesStatsResult,
    bySourceResult,
    ...
    () => loadSection(() => getProfitSummary(), "reports.profit"),
    () => loadSection(() => getExpensesTotal(range.fromDay, range.toDay), "reports.expenses"),
  ...
  const bySource = bySourceResult.ok ? bySourceResult.value : null;
  const profit = profitResult.ok ? profitResult.value : null;
  const expenses = expensesResult.ok ? expensesResult.value : null;
  ...
  // تُحسَب فقط حين توفّر طرفاها معاً. غياب أحدهما يعني أن السطر لا يُعرض
  // أصلاً، لا أنه يساوي صفراً.
  //
  // هذه الثلاثة مشتقّة، فلا يتتبّع TypeScript ضيقها داخل الحارس كما يتتبّع
  // `bySource` و`expenses` أنفسهما. لذلك تظهر `!` عند استعمالها أدناه —
  // وهي في كل موضع داخل فرعٍ حارسه هو نفسه الشرط الذي بُنيت عليه، فلا
  // يُقرأ أيٌّ منها إلا وقد وصل مصدره فعلاً.
  const deliveryNetMad = bySource ? bySource.totals.deliveryNetMad : null;
  const netProfitMad =
    bySource && expenses
      ? bySource.totals.grossProfitMad + bySource.totals.deliveryNetMad - expenses.totalMad
      : null;

  const websiteRow = bySource?.rows.find((row) => row.source === "website");
  const manualRevenue = bySource
    ? bySource.rows
  ...
  return (
    <div>
      ...
      {salesStats === null ? (
        <SectionUnavailable label="مبيعات اليوم و7 أيام والشهر" />
      ) : (
        <div className="mt-3 grid grid-cols-2 gap-3 sm:grid-cols-3">
          ...
          <StatCard
            ...
            value={formatMad(salesStats.salesThisMonthMad)}
          />
        </div>
      )}
      ...
      {bySource === null ? (
        <SectionUnavailable label="المبيعات والأرباح حسب المصدر" />
      ) : (
        <>
          <div className="mt-3 grid grid-cols-2 gap-3 sm:grid-cols-3">
            <StatCard
              label="مبيعات الموقع (المنتجات)"
              value={formatMad(websiteRow?.revenueMad ?? 0)}
            />
            <StatCard label="مبيعات واتساب/يدوية (المنتجات)" value={formatMad(manualRevenue!)} />
            <StatCard
              label="مبيعات المنتجات إجمالاً"
              value={formatMad(bySource!.totals.revenueMad)}
              accent
            />
            <StatCard label="تكلفة البضاعة" value={formatMad(bySource!.totals.cogsMad)} />
            <StatCard
              label="الربح الخام من البضاعة"
              value={formatMad(bySource!.totals.grossProfitMad)}
              accent
            />
            <StatCard
              label="التوصيل المحصَّل من الزبائن"
              value={formatMad(bySource!.totals.deliveryFeesMad)}
            />
          </div>

          {expenses === null ? (
            <SectionUnavailable label="حساب الربح النهائي ومصاريف التشغيل" />
          ) : (
            <>
                  ...
                  {bySource!.totals.ordersMissingDeliveryCost > 0 ? (
                    <>
                      ...
                      <span className="font-semibold">
                        {bySource!.totals.deliveredOrders -
                          bySource!.totals.ordersMissingDeliveryCost}
                      </span>{" "}
            ...
            <p className="mt-2 text-[11px] leading-relaxed text-neutral-500">
              <span className="font-semibold">
                صافي الربح الحقيقي = الربح الخام من البضاعة + صافي أثر التوصيل − مصاريف التشغيل.
              </span>{" "}
          ...
          <p className="mt-2 text-[11px] leading-relaxed text-neutral-500">
            <span className="font-semibold text-neutral-600">عن التوصيل:</span> «التوصيل المحصَّل»
            ما دفعه الزبون، و«صافي التوصيل» هو ما بقي لنا منه بعد طرح ما دفعناه لشركة التوصيل — على
            الطلبات التي سُجِّلت تكلفتها وحدها. السالب يعني أننا نتحمّل جزءاً من التوصيل من ربح
            البضاعة.
            {bySource!.totals.ordersWithMissingCost > 0 && (
              <>
                ...
                ينقص أحد سطوره ثمن الشراء، فيُحتسَب بصفر ويبدو ربحه كامل ثمن البيع. لا نُخمّن ثمناً
                غير مسجَّل.
              </>
            )}
          </p>
      ...
      {profit === null ? (
        <SectionUnavailable label="عدد الطلبات المسلَّمة والملغاة والراجعة، وأرقام الربح الإجمالي" />
      ) : (
        <>
          <div className="mt-3 grid grid-cols-2 gap-3 sm:grid-cols-4">
            <StatCard label="عدد المسلَّمة" value={String(profit!.deliveredOrdersCount)} />
            <StatCard
              label="قيمة المسلَّمة"
              value={formatMad(profit!.deliveredRevenueMad)}
              accent
            />
          ...
          <p className="mt-1 text-xs text-neutral-500">
            الربح = قيمة المنتجات (items_subtotal) − تكلفة الشراء المحفوظة وقت إنشاء كل طلب
            (purchase price snapshot × الكمية)، بدون احتساب مصاريف التوصيل. ربح الطلب لا يتغيّر بعد
            ذلك حتى لو عُدِّل ثمن شراء المنتج لاحقاً.
          </p>
          {profit!.approximateProfitOrdersCount > 0 && (
            <p className="mt-1 rounded-lg bg-amber-50 px-3 py-2 text-xs text-amber-700">
              {profit!.approximateProfitOrdersCount} من الطلبات المسلَّمة أُنشئت قبل حفظ ثمن الشراء
              لحظياً — ربحها هنا تقدير تاريخي بثمن الشراء الحالي فقط، وقد يتغيّر إذا عُدِّل ثمن
              الشراء لاحقاً (موسومة &quot;تقدير&quot; فـجدول آخر الطلبات أسفله).
            </p>
          )}
          <div className="mt-3 grid grid-cols-2 gap-3 sm:grid-cols-3">
            ...
            <StatCard
              label="ربح الشهر الحالي"
              value={formatMad(profit!.profitThisMonthMad)}
              accent
            />
          </div>
          <div className="mt-3 grid grid-cols-2 gap-3">
            ...
            <StatCard
              label="الربح الإجمالي الكلي"
              value={formatMad(profit!.grossProfitMad)}
              accent
            />
          </div>
        </>
      ...
      <div className="mt-6 grid grid-cols-1 gap-4 lg:grid-cols-2">
        <div>
          ...
          {bestByQuantity === null ? (
            <SectionUnavailable label="الأكثر مبيعاً بالكمية" />
        ...
        <div>
          ...
          {bestByValue === null ? (
            <SectionUnavailable label="الأكثر مبيعاً بالقيمة" />
      ...
      {recentDelivered === null ? (
        <SectionUnavailable label="ربح آخر الطلبات المسلَّمة" />
