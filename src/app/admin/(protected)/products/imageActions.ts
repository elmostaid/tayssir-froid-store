 ...
 * فـ/admin/products): يرفع الملف الجديد إلى Supabase Storage (أو محلياً إن
 * لم يكن مُهيَّأ)، ثم يحدّث سجل الصورة الرئيسية الموجود فمكانه (نفس id، يبقى
 * is_primary = true) بدل إنشاء سجل جديد، فلا يمكن أبداً أن ينتج عن هذا سجل
 * "رئيسية" مكرر. إن لم يكن للمنتج أي صورة بعد (حالة نادرة)، يُنشأ سجل واحد
 * جديد كرئيسية.
 *
 * ترتيب آمن صارم:
 *   1) رفع الملف الجديد أولاً والتأكد من نجاحه.
 *   2) تحديث قاعدة البيانات (storage_path فقط). إن فشل، يُحذف الملف الجديد
 *      فوراً (تنظيف) ولا يُلمس السجل/الملف القديم إطلاقاً.
 * الصورة/الملف القديم لا يُحذف ولا يُلمس أبداً — لا هنا ولا بعد نجاح
 * التحديث. هذا مقصود: لا حذف لأي صورة قديمة إطلاقاً مهما كانت النتيجة.
 *
...
export async function replacePrimaryImage(
  ...
  // الملف/الصورة القديمة تبقى بلا حذف أو لمس إطلاقاً — مقصود.
