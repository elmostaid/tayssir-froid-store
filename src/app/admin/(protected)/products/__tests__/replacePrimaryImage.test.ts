...
// PNG 1×1 بكسل صالح فعلياً — يمرّ من validateImageFile (image/png) ويثبت أن
// الملف يُكتب على القرص بمحتوى حقيقي قابل للقراءة.
...
beforeAll(async () => {
  ...
  const [product] = await sql<{ id: number }[]>`
    insert into public.products (
      ...
      'TEST-FIXTURE-CHANGEIMG2', 'test-fixture-changeimg2', ${category.id},
      'منتج اختبار تغيير الصورة 2', 'قطعة', 3, 1, 55.00, 99.00, 40, 'published'
    )
  ...
  await fs.writeFile(oldAbsPath, IMAGE_BYTES);
...
afterEach(async () => {
  ...
  await sql`
    update public.products set
      name_ar = 'منتج اختبار تغيير الصورة 2', purchase_price = 55.00, sale_price = 99.00,
...
afterAll(async () => {
  ...
  await sql`delete from public.product_images where product_id = ${productId}`;
  await sql`delete from public.products where sku = 'TEST-FIXTURE-CHANGEIMG2'`;
});

describe("replacePrimaryImage — تغيير الصورة الرئيسية بخطوة واحدة، آمن للترتيب", () => {
  ...
  test("Admin: فشل تحديث قاعدة البيانات (منتج غير موجود) يحذف الملف الجديد المرفوع تواً ولا يُنشئ أي سجل", async () => {
    getAdminUserMock.mockResolvedValueOnce(ADMIN_USER);
    const nonExistentProductId = 999999999;

    const fd = new FormData();
    fd.set("file", pngFile("orphan-attempt.png"));
    const result = await replacePrimaryImage(nonExistentProductId, { error: null }, fd);

    expect(result.error).toBeTruthy();
    expect(result.success).toBeFalsy();

    // القيد الأجنبي (foreign key) يمنع إدراج سجل لمنتج غير موجود، والفشل
    // يجب أن يُنظِّف الملف المرفوع تواً (لا صور يتيمة على القرص).
    const orphanDir = path.join(process.cwd(), "public", "product-images", String(nonExistentProductId));
    const dirFiles = await fs.readdir(orphanDir).catch(() => [] as string[]);
    expect(dirFiles).toHaveLength(0);

    const rows = await sql<{ id: number }[]>`
      select id from public.product_images where product_id = ${nonExistentProductId}
    `;
    expect(rows).toHaveLength(0);

    // الصورة الرئيسية الحقيقية للمنتج الأصلي لم تُلمس إطلاقاً.
    const originalRows = await sql<{ id: number; storage_path: string }[]>`
      select id, storage_path from public.product_images where product_id = ${productId}
    `;
    expect(originalRows).toHaveLength(1);
    expect(originalRows[0].id).toBe(originalImageId);
    expect(originalRows[0].storage_path).toContain(OLD_STORAGE_PATH_SUFFIX);
    await expect(fs.access(oldAbsPath)).resolves.toBeUndefined();
  });
  ...
  test("Admin: يبدّل الصورة الرئيسية بنجاح — نفس السجل، صورة واحدة رئيسية، الملف الجديد على القرص، والقديم يبقى بلا حذف، وبقية بيانات المنتج بلا تغيير", async () => {
    ...
    `;
    expect(product.name_ar).toBe("منتج اختبار تغيير الصورة 2");
    ...
    const productCount = await sql<{ count: number }[]>`
      select count(*)::int as count from public.products where sku = 'TEST-FIXTURE-CHANGEIMG2'
    `;
