...
export type AbandonedCartsSummary = {
  ...
  abandoned: number;
  /** سلة صغيرة (تحت عتبة القياس) ولم تصل Checkout. */
  stoppedBelowMinimum: number;
  /** سلة كبيرة (فوق العتبة) ولم تصل Checkout — أثمن ما نخسره. */
 ...
 *
 * 2) **العتبة للتصنيف لا للمنع.** حُذف الحد الأدنى للطلب من المتجر، فلم
 *    يعد «تحت 1000» تفسيراً لتوقّف أحد. لكن حذف التقسيم كان سيُفقد المقارنة
 *    مع ما قبل التغيير، فبقيت العتبة **مِسطرةً لقياس حجم السلة** لا حاجزاً:
 *    الفئات صارت تُقرأ «سلة صغيرة / سلة كبيرة»، وهو ما يبقى صالحاً للمقارنة
 *    قبل التغيير وبعده على نفس البيانات التاريخية بلا لمس صفّ واحد.
 *
